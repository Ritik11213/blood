...
if (isset($_POST['apply'])) {
    ...
    $q="SELECT * FROM donars where username='$username'";
    ...
    if (empty($name)) {
        ...
        $error['apply']="Name cannot start with empty spaces";
    }
    else if (empty($age)) {
        ...
        $error['apply']="Empty spaces are not allowed in username";
    }
    else if (empty($email)) {
    ...
    if (count($error)==0) {
        ...
        $query="INSERT INTO donars(name,age,username,password,email,phone,blood_group,gender,country) VALUES('$name','$age','$username','$password','$email','$phone','$blood_group','$gender','$country')";
...
<input type="submit" name="apply"  value="Apply Now" class="btn btn-danger my-2" >
<p>If already have an account <a href="donarlogin.php">Click here</a> </p>
